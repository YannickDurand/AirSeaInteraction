...
for i in range(nbIter-1):
     ...
     #external Force
     totUEXT=np.append(totUEXT,np.sum(uext[0,:,:]*rho0))
     
     #dissipation
     totUDISSh=np.append(totUDISSh,np.sum(udissh*rho0))
     totUDISSv=np.append(totUDISSv,np.sum(udissv*rho0))
     totUDISS=np.append(totUDISS,np.sum(udissh*rho0)+np.sum(udissv*rho0))
     
     #force de pression
     totUPRESS=np.append(totUPRESS,np.sum(upress*rho0))
     totUETA=np.append(totUETA,np.sum(u_eta*rho0))

     #error Model
     totUAB=np.append(totUAB,np.sum(u_ab*rho0))
...
sumTotalDEP=-sumEvoWb-sumEvoDiffTherm[1:]

#Potential Energy
sumDiffEvoPE=sumTotalDEP-sumTotalEPGrid
diffEvoPE=evoPE-totalEP

#total Energie
evoEnergieTotal=evoPE+evoEK
sumEvoTotGrid=sumTotEK+sumTotalEPGrid#+sumEvoWb
...
sumPressionForce=(sumTotUPRESS+sumEvoWb)
pressionForce=(totUPRESS+evoWb2)
sumEvoEner3=sumEvoEner2-sumPressionForce
evoEner3=evoEner2-(pressionForce)

sumEvoEner4=sumEvoEner3-sumTotUETA
#%%
# =============================================================================
# ------------------------Plot Plot Plots-------------------------------------
# =============================================================================
...
"""
zeros=np.zeros(240)
plotComp(915,time[10:],sumDiffEvoPE[10:],sumTotalEPGrid[10:],sumEvoPE[10:],"A","B","C")
#%%
plotComp(9001,time[10:],evoEK[10:],evoPE[10:],evoEnergieTotal[10:],"$D_t(KE)$","$D_t(PE)$","$D_t(TE)$")
#%%
...
#%%
plotComp(903,time[10:],evoEner2[10:],pressionForce[10:],evoEner3[10:],"$D_t(ET)=D_t(TE)-D_t(Diss)$","$PressionForce$","$D_t(ET)-D_tF_{ext}$")

#%%
plotComp(914,time[1:],evoEK[:-1],np.diff(sumTotalEPGrid)/10800.,evoRhoGW[:-1],"$D_t(EK)$","$PEG_{t=i}-PEG_{t=(i-1)}$","$D_t(EP)-RhoWG$")
...
#%%
plotCompT(916,time,sumEvoEner2,sumPressionForce,sumEvoEner3,"$TO_M(t)-TO_M(t=0)$","$Fext(t)-Fext(t=0)$","$diff$")

#%%
plotCompT(917,time,sumEvoEner3,sumTotUETA,sumEvoEner4,"$TO_M(t)-TO_M(t=0)$","$U_{eta}(t)-U_{eta}(t=0)$","$diff$")
